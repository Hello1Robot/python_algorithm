...
while que:
    rx, ry, bx, by, d, cnt, dr = que.popleft()
    ...
    if (nrx, nry, nbx, nby) not in visited:
        ...
        if d in (0, 1): # 상 하 -> 좌우만 추가
            que.append((nrx, nry, nbx, nby, 2, cnt+1, nd))
            que.append((nrx, nry, nbx, nby, 3, cnt+1, nd))
        else: # 좌 우 -> 상하만 추가
            que.append((nrx, nry, nbx, nby, 0, cnt+1, nd))
            que.append((nrx, nry, nbx, nby, 1, cnt+1, nd))
