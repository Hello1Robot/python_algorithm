...
N = int(input())
apples = [tuple(map(int, input().split())) for _ in range(int(input()))]    # 100개까지 주어짐
nxt = [tuple_change(input()) for _ in range(int(input()))]
next = deque(nxt)
dummy = deque()
dummy.append((1,1,))
while dummy:
    ...
    dx, dy = dr[now_d%4]
    nx, ny = x + dx, y + dy
    if nx < 1 or nx > N or ny < 1 or ny > N or (nx, ny,) in dummy:        # 벽에 부딪히거나 내 몸이면 아웃
        break
